...
from .model import Encoder_overall, InfoNCELoss
...
class Train_SpatialGlue:
    ...
    def train(self):
        ...
        self.cl_loss_fn = InfoNCELoss(temperature=self.cl_temperature).to(self.device)
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            # contrastive loss
            loss_cl = 0
            if self.cl_mode in ['cross_modality', 'both']:
                loss_cl += self.cl_loss_fn(results['emb_latent_omics1'], results['emb_latent_omics2'])
            if self.cl_mode in ['cross_graph', 'both']:
                loss_cl += self.cl_loss_fn(results['emb_latent_spatial_omics1'], results['emb_latent_feature_omics1'])
                loss_cl += self.cl_loss_fn(results['emb_latent_spatial_omics2'], results['emb_latent_feature_omics2'])
